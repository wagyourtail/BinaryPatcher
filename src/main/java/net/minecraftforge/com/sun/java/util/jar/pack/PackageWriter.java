...
import static net.minecraftforge.com.sun.java.util.jar.pack.Constants.*;
...
class PackageWriter extends BandStructure {
    ...
    Set<ConstantPool.Entry>                       requiredEntries;  // for the CP
    ...
    void setArchiveOptions() {
        ...
        for (Package.File file : pkg.files) {
...
// with a zipped unbanded file.
    ...
    void writeConstantPool() throws IOException {
        ConstantPool.IndexGroup cp = pkg.cp;
        ...
        for (byte tag : ConstantPool.TAGS_IN_ORDER) {
            ...
            case CONSTANT_Integer:
                for (int i = 0; i < cpMap.length; i++) {
                    ConstantPool.NumberEntry e = (ConstantPool.NumberEntry) cpMap[i];
            ...
            case CONSTANT_String:
                for (int i = 0; i < cpMap.length; i++) {
                    ConstantPool.StringEntry e = (ConstantPool.StringEntry) cpMap[i];
                    cp_String.putRef(e.ref);
                }
                break;
            case CONSTANT_Class:
                for (int i = 0; i < cpMap.length; i++) {
                    ConstantPool.ClassEntry e = (ConstantPool.ClassEntry) cpMap[i];
            ...
            case CONSTANT_NameandType:
                for (int i = 0; i < cpMap.length; i++) {
                    ConstantPool.DescriptorEntry e = (ConstantPool.DescriptorEntry) cpMap[i];
            ...
            case CONSTANT_MethodHandle:
                for (int i = 0; i < cpMap.length; i++) {
                    ConstantPool.MethodHandleEntry e = (ConstantPool.MethodHandleEntry) cpMap[i];
            ...
            case CONSTANT_MethodType:
                for (int i = 0; i < cpMap.length; i++) {
                    ConstantPool.MethodTypeEntry e = (ConstantPool.MethodTypeEntry) cpMap[i];
                    cp_MethodType.putRef(e.typeRef);
                }
                break;
            case CONSTANT_InvokeDynamic:
                for (int i = 0; i < cpMap.length; i++) {
                    ConstantPool.InvokeDynamicEntry e = (ConstantPool.InvokeDynamicEntry) cpMap[i];
            ...
            case CONSTANT_BootstrapMethod:
                for (int i = 0; i < cpMap.length; i++) {
                    ConstantPool.BootstrapMethodEntry e = (ConstantPool.BootstrapMethodEntry) cpMap[i];
    ...
    void writeMemberRefs(byte tag, ConstantPool.Entry[] cpMap, CPRefBand cp_class, CPRefBand cp_desc) throws IOException {
        for (int i = 0; i < cpMap.length; i++) {
        ...
        for (int i = 0; i < ATTR_CONTEXT_LIMIT; i++) {
            ...
            for (int j = 0; j < layoutsAndCounts.length; j++) {
                ...
                if (predefIndex == null) {
                    ...
                    ConstantPool.Entry ne = ConstantPool.getUtf8Entry(def.name());
                    String layout = def.layoutForClassVersion(getHighestClassVersion());
                    ConstantPool.Entry le = ConstantPool.getUtf8Entry(layout);
    ...
    void collectInnerClasses() {
        ...
        Map<ConstantPool.ClassEntry, Package.InnerClass> allICMap = new HashMap<>();
        ...
        Package.InnerClass[] allICs = new Package.InnerClass[allICMap.size()];
    ...
    void writeInnerClasses() throws IOException {
        for (Package.InnerClass ic : pkg.getAllInnerClasses()) {
            int flags = ic.flags;
    ...
    void writeClassesAndByteCodes() throws IOException {
        Package.Class[] classes = new Package.Class[pkg.classes.size()];
    ...
    void writeAttrs(int ctype,
                    ...
                    Package.Class cls) throws IOException {
        MultiBand xxx_attr_bands = attrBands[ctype];
        ...
        for (Attribute a : h.attributes) {
            ...
            if (def.bandCount == 0) {
                if (def == attrInnerClassesEmpty) {
                    ...
                    writeLocalInnerClasses((Package.Class) h);
                    continue;
                }
    ...
    private int initOpVariant(Instruction i, ConstantPool.Entry newClass) {
        ...
        ConstantPool.MemberEntry ref = (ConstantPool.MemberEntry) i.getCPRef(curCPMap);
    ...
    void writeByteCodes(Code code) throws IOException {
        ...
        for (Instruction i = code.instructionAt(0); i != null; i = i.next()) {
            ...
            ConstantPool.Entry ref = i.getCPRef(curCPMap);
            if (ref != null) {
